...
class DroneCamera():
	def __init__(self):
		self.subImage = message_filters.Subscriber("/ardrone/front/image_raw", Image, self.ReceiveImage)
		...
		self.objectf = open("/home/constantin/ros/ardrone_autonomy/bin/output/objectfeatures.txt","a")
		self.envf= open("/home/constantin/ros/ardrone_autonomy/bin/output/envfeatures.txt","a")
	...
	def ReceiveImage(self,data):
		...
		if self.videoFeed==True:
			...
			if self.startHOG==True:
				...
				'''cv2.circle(npImage, (self.x, self.y), 10, (0, 255, 0), -1)
				...
				cv2.putText(npImage,	"oXpos: %.2f" % self.x,(0,60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255))
				cv2.putText(npImage,	"oYpos: %.2f" % self.y,(0,75), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255))
				...
				for winY in xrange((imageHeight-self.hog.winHeight)/self.hog.winStrideY+1):
					for winX in xrange((imageWidth-self.hog.winWidth)/self.hog.winStrideX+1):
						...
						if (startY<=y and y<=endY) and (startX<=x and x<=endX) and p[2]>0:
							self.objectf.write(str(self.hog.computeBlocks(image[startY:endY,startX:endX]))+'\n')
							cv2.imwrite("/home/constantin/ros/ardrone_autonomy/bin/output/samples/"+str((x,y))+".png",image[startY:endY,startX:endX])
